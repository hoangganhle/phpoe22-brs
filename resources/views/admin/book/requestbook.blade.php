                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <table class="table table-hover table-bordered dataTable no-footer" id="sampleTable" role="grid" aria-describedby="sampleTable_info">
                                ...
                                <tbody class="tbody">
                                    @foreach ($requestBook as $req)
                                        <tr role="row" class="odd{{ $req->id }}">
                                            <td class="sorting_1">{{ $req->id }}</td>
                                            <td class="sorting_1">{{ $req->book_name }}</a></td>
                                            <td class="sorting_1">{{ $req->request_content }}</td>
                                            <td class="sorting_1">{{ $req->user->name }}</td>
                                            <td class="sorting_1"><b>
                                                @if ($req->status == \App\Enums\Status::Resolve)
                                                    {{ trans('admin.resolve') }}
                                                @elseif ($req->status == \App\Enums\Status::Processing)
                                                    {{ trans('admin.processing') }}
                                                @endif
                                            </b>
                                            </td>
                                            <td>
                                                <button class="edit-modal btn btn-warning btn-sm" data-id="{{ $req->id }}" data-book_name="{{ $req->book_name }}" data-request_content="{{ $req->request_content }}" data-user_id="{{ $req->user_id }}" data-status="{{ $req->status }}">
